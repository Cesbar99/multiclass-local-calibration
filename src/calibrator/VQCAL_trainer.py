...
class VQCalibrator(pl.LightningModule):
    def __init__(self, vqclassifier, kwargs): #K: int, C: int, lr: float = 1e-3, wd: float = 0.0, learn_pi: bool = False):
        ...
        self.cal = VQDirCal(K=kwargs.models.K, C=kwargs.dataset.num_classes, S=kwargs.models.S, learn_pi=self.learn_pi, learn_bias=self.learn_bias, random=self.random, standard_dirichlet=self.standard_dirichlet)
